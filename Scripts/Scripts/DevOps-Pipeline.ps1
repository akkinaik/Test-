...
if ($refreshToken -and $environmentName) {
    ...
    New-BcEnvironment `
        ...
        -environmentType "Sandbox" | Out-Null
...
if (!$doNotSignApps -and $codeSigncertPfxFile) {
    ...
    if ($ENV:CodeSignCertPfxPassword -ne "") {
        ...
        $codeSignCertPfxPassword = try {
            $ENV:CodeSignCertPfxPassword | ConvertTo-SecureString
        }
        catch {
            ConvertTo-SecureString -String $ENV:CodeSignCertPfxPassword -AsPlainText -Force
        }

        $params["codeSignCertPfxFile"] = $codeSigncertPfxFile
        $params["codeSignCertPfxPassword"] = $codeSignCertPfxPassword
    }
}
...
# Add license only if it exists
if (![string]::IsNullOrWhiteSpace($licenseFile) -and (Test-Path $licenseFile)) {
    Write-Host "Using license file: $licenseFile"
    $runParams["licenseFile"] = $licenseFile
}
else {
    Write-Host "No license file provided. Using sandbox default license."
}

Run-AlPipeline @runParams @params
